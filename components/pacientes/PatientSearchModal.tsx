                ...
                <div className="flex border-b border-slate-100 bg-slate-50 flex-shrink-0">
                    {(['search', 'create'] as const).map(tab => (
                        <button key={tab} onClick={() => setView(tab)}
                            className={`flex-1 py-2.5 text-xs font-black uppercase tracking-wider transition-all border-b-2 ${view === tab ? 'border-[#051650] text-[#051650] bg-white' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>
                            ...
                            <div className="space-y-1.5 max-h-[380px] overflow-y-auto">
                                ...
                                {filtered.length === 0 && search && (
                                    <div className="text-center py-8">
                                        ...
                                        <button onClick={() => setView('create')} className="mt-2 text-[#051650] font-black text-xs uppercase tracking-widest hover:underline">
                                            → Crear nueva ficha
                                        </button>
                                    </div>
                                )}
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                                ...
                                <div className="col-span-2 flex items-center gap-5 p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    ...
                                    <div>
                                        ...
                                        <button type="button" onClick={() => fileRef.current?.click()}
                                            className="mt-2 flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-50 hover:border-[#051650] transition-all">
                            ...
                            <div className="flex gap-3 mt-6 pt-4 border-t border-slate-100">
                                <button type="button" onClick={() => setView('search')}
                                    className="px-5 py-2.5 border border-slate-200 rounded-xl text-xs font-black uppercase text-slate-500 hover:bg-slate-50 transition-all">
                                    Volver
                                </button>
                                <button type="submit"
                                    className="flex-1 py-2.5 bg-[#051650] text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-lg hover:bg-blue-900 active:scale-[0.98] transition-all">
                                    Registrar Paciente
                                </button>
                            </div>
                        </form>
