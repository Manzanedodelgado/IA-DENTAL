    ...
    return (
        <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-4 space-y-4 animate-in fade-in duration-150">
            <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] font-black text-[#051650] uppercase tracking-widest flex items-center gap-1.5"><Settings2 className="w-3.5 h-3.5 text-[#0056b3]" />Configuración</span>
                ...
                <div className="flex gap-1.5 flex-wrap">
                    {Object.entries(CH).map(([key, v]) => (
                        <button key={key} onClick={() => setCfg(p => ({ ...p, channel: key }))}
                            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${cfg.channel === key ? 'bg-[#0056b3] text-white' : 'bg-white border border-slate-200 text-slate-500 hover:border-[#0056b3]/30'}`}>
            ...
            <div>
                ...
                <input value={cfg.conditions} onChange={e => setCfg(p => ({ ...p, conditions: e.target.value }))}
                    ...
                    className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-[11px] focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20" />
            </div>
            ...
            <div>
                    ...
                    <div className="flex gap-1 flex-wrap justify-end">
                        {VARS.map(v => (
                            <button key={v} onClick={() => setCfg(p => ({ ...p, message: p.message + ' ' + v }))}
                                className="text-[9px] font-black px-1.5 py-0.5 bg-[#0056b3]/10 text-[#0056b3] rounded border border-[#0056b3]/20 hover:bg-[#0056b3]/20 transition-all font-mono">
                                {v}
                            </button>
                        ))}
                    </div>
                ...
                <textarea value={cfg.message} onChange={e => setCfg(p => ({ ...p, message: e.target.value }))} rows={4}
                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-[11px] text-slate-700 font-mono focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 resize-none" />
            ...
            <div className={`bg-white rounded-xl border transition-all ${a.active ? 'border-slate-200' : 'border-slate-100 opacity-60'} ${expanded || configOpen ? 'shadow-md ring-1 ring-[#0056b3]/10' : 'hover:shadow-sm'}`}>
                <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={onExpand}>
                    <div className="flex-1 min-w-0">
                        <span className="text-[12px] font-black text-[#051650] block truncate">{a.name}</span>
                        <div className="flex items-center gap-3 text-[10px] text-slate-400 mt-0.5">
                            ...
                            <span className="text-[#0056b3] font-black">{a.successRate}% éxito</span>
                        </div>
                    </div>
                    <button onClick={e => { e.stopPropagation(); onToggle(); }}>
                        {a.active ? <ToggleRight className="w-7 h-7 text-[#0056b3]" /> : <ToggleLeft className="w-7 h-7 text-slate-300" />}
                    </button>
                    ...
                    <div className="border-t border-slate-100 px-4 pb-4 pt-3 space-y-3 animate-in fade-in duration-150">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-[10px]">
                            <div className="bg-slate-50 rounded-lg p-2 border border-slate-100">
                                ...
                                <p className="font-black text-[#051650]">{a.config.delayValue} {a.config.delayUnit}</p>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-2 border border-slate-100">
                                <p className="text-slate-400 font-bold uppercase text-[9px]">Horario</p>
                                <p className="font-black text-[#051650] truncate">{a.config.schedule}</p>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-2 border border-slate-100">
                                <p className="text-slate-400 font-bold uppercase text-[9px]">Envíos</p>
                                <p className="font-black text-[#051650]">{a.executions}</p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            ...
                            <button className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-black uppercase border border-[#0056b3]/20 text-[#0056b3] rounded-lg hover:bg-[#0056b3]/5 transition-all">
                ...
                return (
                    <div key={g.key} className={`rounded-2xl border ${g.border} ${g.bg} overflow-hidden`}>
                        <div className="flex items-center justify-between px-4 py-2.5 cursor-pointer" onClick={() => toggleGroup(g.key)}>
                            <div className="flex items-center gap-3">
                                <span className="text-[12px] font-black text-[#051650]">{g.label}</span>
