        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                <input value={search} onChange={e => setSearch(e.target.value)}
                    placeholder="Buscar plantilla..." className="bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-[11px] focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 w-56" />
                <div className="flex gap-1.5 flex-wrap">
                    <button onClick={() => setFilterType('all')} className={`px-2.5 py-1 rounded-xl text-[10px] font-black uppercase transition-all ${filterType === 'all' ? 'bg-[#0056b3] text-white' : 'bg-white border border-slate-200 text-slate-400 hover:border-[#0056b3]/30'}`}>Todas</button>
                    {(Object.keys(TYPE_CONFIG) as TemplateType[]).map(type => {
                        const tc = TYPE_CONFIG[type]; const Icon = tc.icon;
                        return (
                            <button key={type} onClick={() => setFilterType(type)} className={`flex items-center gap-1 px-2.5 py-1 rounded-xl text-[10px] font-black uppercase transition-all ${filterType === type ? 'bg-[#0056b3] text-white' : 'bg-white border border-slate-200 text-slate-400 hover:border-[#0056b3]/30'}`}>
            ...
            {editing ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
                    <div className="flex items-center justify-between">
                        <span className="text-[12px] font-black text-[#051650] uppercase tracking-widest flex items-center gap-2"><Edit3 className="w-4 h-4 text-[#0056b3]" />Editando: {editing.name}</span>
                        <button onClick={() => setEditing(null)} className="text-slate-300 hover:text-slate-500"><X className="w-4 h-4" /></button>
                    </div>
                    <div>
                        <label className="block text-[9px] font-black text-slate-400 uppercase tracking-wider mb-1">Nombre</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1">
                            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Contenido</label>
                            <div className="flex gap-1 flex-wrap justify-end">
                                {['{{nombre}}', '{{fecha}}', '{{hora}}', '{{doctor}}', '{{tratamiento}}', '{{importe}}', '{{enlace}}'].map(v => (
                                    <button key={v} onClick={() => setEditing({ ...editing, content: editing.content + ' ' + v })}
                                        className="text-[9px] font-black px-1.5 py-0.5 bg-[#0056b3]/10 text-[#0056b3] rounded border border-[#0056b3]/20 hover:bg-[#0056b3]/20 font-mono">{v}</button>
                                ))}
                            </div>
                        </div>
                        <textarea value={editing.content} onChange={e => setEditing({ ...editing, content: e.target.value })} rows={12}
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-[11px] text-slate-700 font-mono focus:outline-none focus:ring-2 focus:ring-[#0056b3]/20 resize-none" />
                    </div>
                    <button onClick={() => handleSave(editing)} className="flex items-center gap-2 px-4 py-2 bg-[#0056b3] text-white rounded-xl text-[10px] font-black uppercase hover:bg-[#004494] transition-all">
                ...
                categories.map(cat => {
                    ...
                    return (
                        <div key={cat} className="space-y-2">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">{cat}</p>
                            {items.map(t => {
                                    ...
                                    <div key={t.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                                        <div className="flex items-center gap-3 px-4 py-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[12px] font-black text-[#051650] truncate">{t.name}</p>
                                                <p className="text-[10px] text-slate-400 truncate">{t.content.split('\n')[0].slice(0, 60)}…</p>
                                            </div>
                                            <div className="flex gap-1.5 shrink-0">
                                                <button onClick={() => copyTemplate(t)} className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition-all ${copied === t.id ? 'bg-emerald-500 text-white' : 'border border-slate-200 text-slate-500 hover:border-[#0056b3]/30 hover:text-[#0056b3]'}`}>
                                                    {copied === t.id ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                                    {copied === t.id ? '¡Copiado!' : 'Copiar'}
                                                </button>
                                                <button onClick={() => setEditing(t)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-200 text-[10px] font-bold text-slate-500 hover:border-[#0056b3]/30 hover:text-[#0056b3] transition-all">
